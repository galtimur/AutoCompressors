...
def eval_model(
    ...
    if model_name.startswith("base_model"):
        model, modules["tokenizer"] = load_base_model(
            "deepseek-ai/deepseek-coder-1.3b-base"
        )
        modules["run_config"] = None
    else:
        model, modules["tokenizer"], modules["run_config"] = load_model_from_ckpt(
            checkpoint_path
        )
    ...
    if do_lcc:
        result_lcc = eval_on_lcc(modules, ds_test, context_size, limit)
...
if __name__ == "__main__":
    ...
    run_benchmark(
        ...
        results_path,
        limit=2000,
        limit_loss_samples=1000,
        do_lcc=True,
        do_ce_loss=True,
        copy_last_chunck=False,
    )
